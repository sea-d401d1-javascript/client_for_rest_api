...
describe('two resources controller', () => {
  ...
  it('should be able to make a controller', function() {
    var TwoResourcesController = $ControllerConstructor('TwoResourcesController',{
      $scope
    });
    expect(typeof TwoResourcesController).toBe('object');
    expect($scope.movies.length).toBe(0);
    expect($scope.actors.length).toBe(0);
    expect($scope.movie).toBe(true);
    expect($scope.actor).toBe(true);
  });

  describe('REST request', () => {
    beforeEach(angular.mock.inject(function(_$httpBackend_){
      $httpBackend = _$httpBackend_;
      $ControllerConstructor('TwoResourcesController', {
        $scope
      });
    }));
    ...
    it('should make a get request to /api/movies', () => {
      ...
      expect($scope.movies[0].name).toBe('testmovie');
    });

    it('should make a get request to  /api/actors', () => {
    ...
    it('should make a post request to /api/movies', () => {
    ...
    it('should make a post request to /api/actors', () => {
